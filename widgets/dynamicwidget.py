...
class DynamicLineWidget():
    ...
    def __init__(self, **kwargs):
        ...
        try:
            self.scaled_bezier = list(x / (w if not i % 2 else h) for i, x in enumerate(kwargs['bezier']))
            del self.kwargs['bezier']
        except:
            pass

        try: 
            self.scaled_circle = [kwargs['circle'][0] / w, kwargs['circle'][1] / h, kwargs['circle'][2] / (w + h)]
    ...
    def redraw(self):
        ...
        try:
            new_bezier = list(x * (w if not i % 2 else h) for i, x in enumerate(self.scaled_bezier))
        ...
        try:
            new_circle = [self.scaled_circle[0] * w, self.scaled_circle[1] * h, self.scaled_circle[2] * (w + h)]
            with self.parent.canvas:
